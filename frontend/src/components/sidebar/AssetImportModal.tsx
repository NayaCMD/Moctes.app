...
import type { AssetType, LibraryAsset } from "../../types/asset.types";
import { MAX_ASSET_FILE_SIZE, validateAssetFile } from "../../utils/assetLibrary.utils";
...
export function AssetImportModal({ open, initialType, svgMode, onClose }: AssetImportModalProps) {
  ...
  const setImportStatus = useAssetLibraryStore((state) => state.setImportStatus);
  ...
  const [svgImportMode, setSvgImportMode] = useState<"shape" | "sticker">(svgMode ?? "sticker");
  ...
  const canImport = useMemo(
    () => Boolean((sanitizedFile ?? file) && name.trim() && folderId && !errorMessage),
    [errorMessage, file, folderId, name, sanitizedFile],
  );
  ...
  const handleFile = async (selectedFile: File | null) => {
    ...
    if (selectedFile.type === "image/svg+xml") {
      ...
      if (sanitized.ok === false) {
        ...
        setImportStatus("error", { code: "INVALID_FILE_TYPE", message: sanitized.message });
        return;
      }
      fileForStorage = new File([sanitized.text], selectedFile.name, { type: "image/svg+xml" });
      setType("sticker");
      setSvgImportMode(svgMode ?? "sticker");
    }
  ...
  const importAsset = async () => {
    ...
    try {
      await indexedDbAssetStorage.saveAssetBlob(assetId, fileForStorage);
      setSelectedAsset(assetId);
      setFeedbackMessage("Asset importado.");
      closeAndReset();
    } catch {
      useAssetLibraryStore.getState().deleteAsset(assetId);
      const storageError = { code: "STORAGE_ERROR" as const, message: "Não foi possível salvar o arquivo." };
  ...
  return createPortal(
    <div className="asset-modal-backdrop" role="presentation" onMouseDown={closeAndReset}>
      <form
      ...
      >
        <header>
          ...
          <button type="button" aria-label="Fechar importação" onClick={closeAndReset}>
            Fechar
          </button>
        </header>
        ...
        <label className="asset-file-picker">
          ...
          {errorMessage && <span className="asset-field-error">{errorMessage}</span>}
        </label>
        ...
        <div className="asset-import-preview">
          {previewUrl ? <img src={previewUrl} alt="Preview do asset" /> : <span>Escolha um arquivo</span>}
        </div>
        <p className="asset-import-meta">
          {file ? `${file.type} - ${Math.round(file.size / 1024)} KB` : `Limite: ${MAX_ASSET_FILE_SIZE / 1024 / 1024} MB`}
