...
import { defaultAssetFolders, defaultLibraryAssets } from "../data/initialAssetLibrary";
import type {
  AssetFolder,
  AssetImportStatus,
  AssetLibraryError,
...
export type AssetLibraryResult<T = void> =
  | { ok: true; value: T }
  | { ok: false; error: AssetLibraryError };
...
interface AssetLibraryStoreState {
  ...
  deleteFolder: (folderId: string, destinationFolderId?: string) => AssetLibraryResult<string[]>;
  ...
  updateAsset: (assetId: string, updates: Partial<LibraryAsset>) => AssetLibraryResult;
  ...
  setImportStatus: (status: AssetImportStatus, error?: AssetLibraryError | null) => void;
...
type PersistedAssetLibraryState = Pick<
  ...
  "folders" | "assets" | "activeFolderId" | "selectedAssetId" | "searchQuery" | "activeTypeFilter"
>;
    ...
    (set, get) => ({
      ...
      createFolder: (name) => {
        ...
        if (state.folders.length >= MAX_ASSET_FOLDERS) {
          return failure("FOLDER_LIMIT_REACHED", "Limite de 5 pastas atingido.");
        }

        const trimmedName = (name?.trim() || `Pasta ${state.folders.length + 1}`).slice(0, 40);
        if (!trimmedName) {
      ...
      renameFolder: (folderId, name) => {
        ...
        set((state) => ({
          folders: state.folders.map((folder) =>
            folder.id === folderId ? { ...folder, name: trimmedName, updatedAt: timestamp() } : folder,
          ),
      ...
      deleteFolder: (folderId, destinationFolderId) => {
        ...
        if (state.folders.length <= 1) {
          return failure("FOLDER_LIMIT_REACHED", "Crie outra pasta antes de excluir esta.");
        }

        const folderAssets = state.assets.filter((asset) => asset.folderId === folderId);
        ...
        if (destinationFolderId && !destination) {
          return failure("FOLDER_NOT_FOUND", "Pasta de destino nao encontrada.");
        }
        if (destination) {
          const destinationCount = state.assets.filter((asset) => asset.folderId === destination.id).length;
          if (destinationCount + folderAssets.length > MAX_ASSETS_PER_FOLDER) {
            return failure("ASSET_LIMIT_REACHED", "A pasta de destino atingiria o limite de 15 itens.");
          }
        }

        const folders = normalizeFolderOrder(state.folders.filter((item) => item.id !== folderId));
        const nextActiveFolderId =
          state.activeFolderId === folderId ? folders[0].id : state.activeFolderId;
        set({
          folders,
          assets: destination
            ? state.assets.map((asset) =>
                asset.folderId === folderId
                  ? { ...asset, folderId: destination.id, updatedAt: timestamp() }
                  : asset,
              )
            : state.assets.filter((asset) => asset.folderId !== folderId),
          activeFolderId: nextActiveFolderId,
          selectedAssetId:
            state.selectedAssetId && folderAssets.some((asset) => asset.id === state.selectedAssetId)
              ? null
              : state.selectedAssetId,
          feedbackMessage: destination ? "Pasta excluida e assets movidos." : "Pasta e assets excluidos.",
        });
      ...
      reorderFolders: (folderIds) =>
        set((state) => {
          const orderById = new Map(folderIds.map((folderId, index) => [folderId, index + 1]));
          return {
      ...
      addAsset: (asset) => {
        ...
        if (state.assets.filter((item) => item.folderId === asset.folderId).length >= MAX_ASSETS_PER_FOLDER) {
          return failure("ASSET_LIMIT_REACHED", "Esta pasta ja possui 15 itens.");
        }
      ...
      moveAssetToFolder: (assetId, folderId) => {
        ...
        if (state.assets.filter((asset) => asset.folderId === folderId).length >= MAX_ASSETS_PER_FOLDER) {
          return failure("ASSET_LIMIT_REACHED", "Esta pasta ja possui 15 itens.");
        }
        set({
          assets: state.assets.map((asset) =>
            asset.id === assetId ? { ...asset, folderId, updatedAt: timestamp() } : asset,
          ),
      ...
      setImportStatus: (importStatus, importError = null) => set({ importStatus, importError }),
...
function failure(code: AssetLibraryError["code"], message: string): AssetLibraryResult<never> {
...
function sortAssets(assets: LibraryAsset[], sortMode: AssetSortMode): LibraryAsset[] {
...
function sanitizePersistedAssetLibraryState(value: unknown): PersistedAssetLibraryState {
  if (!isPersistedAssetLibraryState(value)) {
    return fallbackState;
  }

  const folders = value.folders.length > 0 ? normalizeFolderOrder(value.folders).slice(0, MAX_ASSET_FOLDERS) : defaultAssetFolders;
  const folderIds = new Set(folders.map((folder) => folder.id));
  const assets = value.assets
    .filter((asset) => folderIds.has(asset.folderId))
    .filter((asset, _index, all) => all.filter((item) => item.folderId === asset.folderId).indexOf(asset) < MAX_ASSETS_PER_FOLDER);
  const activeFolderId = folderIds.has(value.activeFolderId) ? value.activeFolderId : folders[0].id;

  return {
    folders,
    assets: assets.length > 0 ? assets : defaultLibraryAssets,
    activeFolderId,
    selectedAssetId: assets.some((asset) => asset.id === value.selectedAssetId) ? value.selectedAssetId : null,
    searchQuery: value.searchQuery ?? "",
    activeTypeFilter: value.activeTypeFilter ?? "all",
  };
}

function isPersistedAssetLibraryState(value: unknown): value is PersistedAssetLibraryState {
  return (
    typeof value === "object" &&
    value !== null &&
    "folders" in value &&
